...
\vspace{20mm}
{\Large (cdd+ Version 0.73,  September 6, 1995)}

{\Large (Manual  Version  September 6, 1995)}
...
other available programs, such as the reverse search code rs or
...
I will be happy to include an application description in future
...
sign.  Note that the full list would be $5 \; : \; 1 \; 2 \; 3 \; 4 \; 6$, where
...
The incidence file (adjacency file, input\_adjacency as well) 
...
\end{verbatim}
Observe that the artificially added 6th inequality (infinity)
is redundant because the first four facets intersects at
...
It is often more desireble to compute the adjacency, 
input\_adjacency and incidence relations
...
For the moment, the LP solver is primitive:
one can use either the dual simplex
...
The default names are {\em test.ext\/}, {\em test.solved\/}, {\em test.icd\/},
 {\em test.adj\/}, {\em test.iad\/},
{\em testsub.ine\/} for the extreme points/ray file, 
the input verification file, the incidence
file, the adjacency file, the input\_adjacency 
and the preprojection variable sub-inequality
